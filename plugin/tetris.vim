...
fu! s:Put(l,c,pos,m)
  " rotations to match official Tetris Guidelines
...
fu! s:End()
  ...
  if line('$')<10 || matchstr(getline('$'),'\d\+$')<s:score
    let numlen=20-strlen(s:score) "highscores don't wrap around when shown
    setl ve=all ma
    cal append('$',s:name)
...
fu! s:Init()
  ...
  let s:ow=bufnr('%')
  let s:score=0
  " tetris opens below instead of top of window
  exe 'sp '.escape(s:s,' ').'|set ma|1,$d'

  let b:col=0
  let b:sh=0
  let b:nsh=(localtime()+8*b:sh)%s:shs "first shape is random
  let b:ncol=b:nsh  "first shape color matches with shape
  let b:pos=0
  let b:x=6
  let b:y=20
  ...
  "colors match official Tetris colors as close as possible.
  if s:background
...
fu! s:Loop(c)
  ...
  elsei c=="\<Esc>" || c=='q'
    ...
    retu 2
  elsei c=~'[+=]'
    if s:COUNTER-10>0  "check to prevent infinite speed
      let s:COUNTER=s:COUNTER-5  "make speed change faster
    en
  en
...
fu! s:Main()
  ...
  wh 1
    ...
    let b:ncol=b:nsh "each shape has a specific color
